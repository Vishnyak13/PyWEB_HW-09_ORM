...
def create_fake_contacts(count: int) -> None:
    for _ in range(count):
        contact = Contact(first_name=fake.first_name(), last_name=fake.last_name(),
                          address=fake.address(), created_at=fake.date_between(start_date='-1y', end_date='today'))
        session.add(contact)
        session.commit()
...
def get_contact_by_id(contact_id):
    contact = session.query(Contact).options(joinedload(Contact.phone), joinedload(Contact.email)).filter(
...
def get_all_contacts():
    contacts = session.query(Contact).options(joinedload(Contact.phone), joinedload(Contact.email)).all()
    contacts_list = []
